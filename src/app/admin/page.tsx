...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"generate" | "submissions" | "shop" | "arc" | "series">("generate");
  ...
  async function loadSeriesList() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to load series"); return; }
  ...
  async function saveSeriesList() {
    ...
    } catch { setError("Failed to save series"); }
  ...
  async function createNewSeries() {
    ...
    setError("");
    const id = newSeriesTitle.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-");
    try {
      const res = await fetch("/api/series", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to create series"); return; }
      setNewSeriesTitle("");
  ...
  // Load series list on first auth for the selector
  const seriesForSelector = allSeries.length > 0 ? allSeries : [
      ...
      {state !== "locked" && (
        <div className="mt-6 mb-8 flex gap-1 rounded-xl border border-border p-1">
          ...
          <button
            onClick={() => {
              setTab("series");
              if (allSeries.length === 0) loadSeriesList();
            }}
            className={`flex-1 rounded-lg px-4 py-2 text-sm font-semibold transition-colors cursor-pointer ${
              tab === "series"
                ? "bg-[rgba(212,137,58,0.15)] text-[#D4893A]"
        ...
        <div className="mt-8">
          <div className="mb-6 flex items-center justify-between">
            ...
            <Button
              ...
              className={savingSeries || allSeries.length === 0 ? "opacity-50 pointer-events-none" : ""}
            >
              {savingSeries ? "Saving..." : "Save to GitHub"}
            </Button>
          </div>
          ...
          <div className="mb-8 rounded-xl border border-border bg-surface p-5">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                className={creatingSeries || !newSeriesTitle.trim() ? "opacity-50 pointer-events-none" : ""}
              >
          ...
          {!loadingSeries && allSeries.length > 0 && (
            <div className="space-y-4">
              {allSeries.map((s, i) => (
                <div
                  key={s.id}
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Title" value={s.title} onChange={(v) => updateSeriesMeta(i, "title", v)} />
                    <Field label="Tagline" value={s.tagline} onChange={(v) => updateSeriesMeta(i, "tagline", v)} />
                  </div>
                  ...
                  <Field label="Description" value={s.desc} onChange={(v) => updateSeriesMeta(i, "desc", v)} multiline rows={2} />
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            updateSeriesMeta(i, "bg", `linear-gradient(135deg, ${e.target.value} 0%, ${e.target.value}88 50%, ${e.target.value}44 100%)`);
                          }}
